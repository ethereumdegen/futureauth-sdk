    ...
    S: Clone + Send + Sync + 'static + AsRef<Arc<FutureAuth>>,
{
    Router::new()
        ...
        .route("/api/auth/send-magic-link", post(send_magic_link))
        .route("/api/auth/verify-magic-link", post(verify_magic_link))
...
async fn send_magic_link(
    ...
    match futureauth.send_magic_link(&body.email).await {
        Ok(()) => (StatusCode::OK, Json(serde_json::json!({ "ok": true }))).into_response(),
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: "Failed to send magic link".into(),
                }),
            )
                .into_response()
        }
...
async fn verify_magic_link(
    ...
    match futureauth
        .verify_magic_link(&body.token, ip.as_deref(), ua.as_deref())
        .await
    {
        Ok((user, session)) => {
            let cookie_name = &futureauth.config.cookie_name;
        ...
        Err(crate::FutureAuthError::InvalidMagicLink | crate::FutureAuthError::MagicLinkExpired) => (
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: "Invalid or expired magic link".into(),
            }),
        )
            .into_response(),
        Err(e) => {
            tracing::error!("Magic link verification failed: {e}");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: "Verification failed".into(),
